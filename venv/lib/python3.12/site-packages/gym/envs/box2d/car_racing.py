...
class CarRacing(gym.Env, EzPickle):
    ...
    def _render_indicators(self, W, H):
        ...
        def render_if_min(value, points, color):
            if abs(value) > 1e-4:
                # Convert all points to standard float if they are numpy float types
                points = [(float(x), float(y)) if isinstance(x, (np.float32, np.float64)) or isinstance(y, (
                np.float32, np.float64)) else (x, y) for x, y in points]
